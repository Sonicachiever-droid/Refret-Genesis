...
private struct StartupSequenceView: View {
    ...
    var body: some View {
        ...
        let fontSize: CGFloat = state.phase == .armed ? 42 : 34
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack {
                ...
                .allowsHitTesting(false)
                .opacity((codenameNemoEnabled || isCodeScreensaverMode) ? 0 : 1)
            ...
            .onReceive(Timer.publish(every: 0.1, on: .main, in: .common).autoconnect()) { date in
                guard isCodeScreensaverMode, startupSequenceActivated else { return }
