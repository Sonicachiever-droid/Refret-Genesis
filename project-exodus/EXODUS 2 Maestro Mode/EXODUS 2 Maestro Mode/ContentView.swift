...
private struct StringPromptLightRow: View {
    let containerSize: CGSize
    let neckWidth: CGFloat
    let centerY: CGFloat
    let activeStrings: [Int]

    var body: some View {
        let grooveCenters = GuitarStringLayout.stringCenters(containerWidth: containerSize.width, neckWidth: neckWidth)
        let activeSet = Set(activeStrings)
        let minimumCenterSpacing = grooveCenters.enumerated().dropFirst().map { idx, center in
            center - grooveCenters[idx - 1]
        }.min() ?? 24
        let lightDiameter: CGFloat = min(max(minimumCenterSpacing * 0.72, 8), 18)
        let labelFontSize: CGFloat = min(max(lightDiameter * 0.7, 8), 12)
        let labelOffsetY: CGFloat = lightDiameter * 0.95
        ...
        return ZStack {
            ForEach(0..<GuitarStringLayout.totalStrings, id: \.self) { index in
                ...
                let lightX = grooveCenters[index]

                Circle()
                    .fill(isActive ? Color(red: 1.0, green: 0.7, blue: 0.12) : Color.white.opacity(0.18))
                    .frame(width: lightDiameter, height: lightDiameter)
                    .overlay(
                        Circle()
                            .stroke(isActive ? Color.white.opacity(0.75) : Color.white.opacity(0.24), lineWidth: 0.8)
                    )
                    .shadow(color: isActive ? Color(red: 1.0, green: 0.55, blue: 0.0).opacity(0.55) : .clear, radius: 6)
                    .position(x: lightX, y: centerY)

                Text("\(stringNumber)")
                    .font(.system(size: labelFontSize, weight: .bold, design: .monospaced))
                    .foregroundStyle(isActive ? Color.white.opacity(0.96) : Color.white.opacity(0.62))
                    .position(x: lightX, y: centerY + labelOffsetY)
            }
        }
...
struct ContentView: View {
    ...
    private let maestroNeckScaleFactor: CGFloat = 0.5
    private let maestroViewingWindowHeightMultiplier: CGFloat = 2.0
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            let baseNeckWidth = (proxy.size.width - padding * 2) * 0.8
            let neckWidth = baseNeckWidth * maestroNeckScaleFactor
            ...
            let neckClipFrameHeight = proxy.size.height * 0.96
            let visibleClipHeight = neckClipFrameHeight * maestroNeckScaleFactor
            let unclippedHeight = visibleClipHeight / visibleRatio
            let minimumNeckHeight = proxy.size.height * 1.35 * maestroNeckScaleFactor
            ...
            let highlightHeight = 2 * gridRowHeight * maestroNeckScaleFactor * maestroViewingWindowHeightMultiplier
            ...
            let sideStringLabel = isGameplayStarted ? "\(max(currentRound, 0))" : ""
            let sideStringScreenWidth = max(screenBannerHeight * 0.82, 34)
            let sideStringScreenHeight = screenBannerHeight * 0.92
            let sideScreenInsetFromPhoneEdge = max(padding * 0.3, 6)
            let leftWindowEdgeX = (proxy.size.width / 2) - (highlightWidth / 2)
            let rightWindowEdgeX = (proxy.size.width / 2) + (highlightWidth / 2)
            let leftPhoneInnerEdgeX = sideScreenInsetFromPhoneEdge
            let rightPhoneInnerEdgeX = proxy.size.width - sideScreenInsetFromPhoneEdge
            let leftSideStringScreenX = (leftPhoneInnerEdgeX + leftWindowEdgeX) / 2
            let rightSideStringScreenX = (rightWindowEdgeX + rightPhoneInnerEdgeX) / 2
            ...
            let stringPromptLightRowY = min(max(windowBottomY + max(gridRowHeight * 0.24, 12), 0), upperWhitePipingY - max(gridRowHeight * 0.18, 10))
            ...
            let topStatusOuterWidth = baseNeckWidth + max(highlightAvailableWidth - baseNeckWidth, 0) / 2
            ...
            ZStack {
                ...
                HStack {
                    ...
                    .frame(width: neckWidth, height: neckClipFrameHeight)
                    .clipped()
                    Spacer()
                }
                ...
                if showFretboardGuide && !isCodeScreensaverMode {
                    let guideBoxHeight = topStatusOuterHeight * 0.5 * maestroNeckScaleFactor
                ...
                if shouldShowQuestionUI {
                    MiniTVFrame(
                        text: sideStringLabel,
                        width: sideStringScreenWidth,
                        height: sideStringScreenHeight,
                        fontScale: 0.82,
                        glowTint: questionBoxAssistActive ? .orange : nil,
                        hitTestingEnabled: false
                    )
                    .position(x: leftSideStringScreenX, y: orangeGreenUnitCenterY)
                    .allowsHitTesting(false)
                    ...
                    .position(x: rightSideStringScreenX, y: orangeGreenUnitCenterY)
                    .allowsHitTesting(false)
                    .opacity(codenameNemoEnabled ? 0 : initialGameplayDimOpacity * introScale)
                    ...
                    WhiteNoteBoxOverlay(
                        ...
                        boxHeight: gridRowHeight * 0.9 * maestroNeckScaleFactor,
                        neckWidth: neckWidth,
                    ...
                    StringPromptLightRow(
                        containerSize: proxy.size,
                        neckWidth: neckWidth,
                        centerY: stringPromptLightRowY,
                        activeStrings: activePickedStringNumbers
                    )
                    .opacity(codenameNemoEnabled ? 0 : initialGameplayDimOpacity * introScale)
                }
